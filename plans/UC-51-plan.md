...
The feature provides an Administrator UI for rejecting a Tour Operator application with a mandatory reason modal prompt (`MSG115`), client-side validation, error handling, and toast notification (`MSG116`).
...
3. **API Endpoint**: `POST /api/v1/admin/tour-operator-applications/{userId}/reject`.
4. **Toast & Modal**: Follows design system components (`RejectModal.tsx`, `FeedbackAlert.tsx`).
