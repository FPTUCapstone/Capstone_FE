...
describe('PublicNavigation S01 restore-aware runtime', () => {
  ...
  it.each([
    ...
    render(<PublicNavigation />);
    await waitFor(() => expect(screen.getByRole('link', { name: 'Partner' })).toBeDefined());
    expect(screen.getByRole('link', { name: 'Partner' }).getAttribute('href')).toBe(href);
  });
  ...
  it('preserves an unresolved TourOperator as the fail-closed application projection, not Approved', async () => {
    ...
    render(<PublicNavigation />);
    await waitFor(() => expect(screen.getByRole('link', { name: 'Partner' })).toBeDefined());
    expect(screen.getByRole('link', { name: 'Partner' }).getAttribute('href')).toBe('/partner/application');
  });
  ...
  it('settles unauthenticated when there is no refresh cookie, exposing guest registration', async () => {
    ...
    render(<PublicNavigation />);
    await waitFor(() => expect(screen.getByRole('link', { name: 'Partner' })).toBeDefined());
    expect(screen.getByRole('link', { name: 'Partner' }).getAttribute('href')).toBe('/partner/register');
