...
// Mirrors TripMate.Domain.Common.AuditActionTypes (backend contract).
const ACTION_TYPE_OPTIONS = ['ApproveOperatorApplication', 'POI_CREATE'] as const;

// Mirrors TripMate.Domain.Common.AuditEntityTypes (backend contract).
const AFFECTED_ENTITY_OPTIONS = ['OperatorProfile', 'POI'] as const;

const ACTOR_ROLE_OPTIONS: UserRole[] = ['Administrator', 'TourOperator', 'Traveler'];

export function AuditLogFilterBar({ filters, onChange, onReset }: AuditLogFilterBarProps) {
  const fromDateRef = useRef<HTMLInputElement>(null);
  const toDateRef = useRef<HTMLInputElement>(null);

  // CR-02: keyword search applies on submit, not on each keystroke.
  ...
  const handleKeywordSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    onChange({ keyword: trimmed || undefined });
  };
  ...
  // Local (Asia/Ho_Chi_Minh) calendar date, not the UTC date.
  ...
  return (
    <div className="rounded-xl border border-[#314863] bg-[#102a43] p-4 text-slate-100 shadow-md">
      <form onSubmit={handleKeywordSubmit}>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <div>
            ...
            >
              ...
              {AFFECTED_ENTITY_OPTIONS.map((entity) => (
        ...
        {/* Submit & Reset Action Bar (CR-02: search applies on submit) */}
        <div className="mt-4 flex items-center justify-end gap-2 border-t border-[#314863]/50 pt-3">
          <button
            type="submit"
            className="inline-flex items-center gap-1.5 rounded-lg bg-[#006b5f] px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-[#004d44]"
          >
