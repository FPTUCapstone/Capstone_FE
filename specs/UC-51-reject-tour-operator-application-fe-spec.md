...
│   ├── RejectModal.tsx               <-- UC-51 Rejection Modal
│   └── StatusBadge.tsx
├── services/
│   └── tourOperatorAdminService.ts   <-- API integration
└── types/
...
1. **Trigger:** On the Application Detail screen (`/admin/tour-operator-applications/[id]`), the Administrator clicks "Reject".
...
4. **API Integration:**
   - Calls `POST /api/v1/admin/tour-operator-applications/{id}/reject` with `{ reason: string }`.
   - Disables submit button and shows loading spinner during request.
5. **Success Handling:**
   - Shows success toast: `"Application rejected. Notification sent to operator."` (`MSG116`).
   - Closes modal.
